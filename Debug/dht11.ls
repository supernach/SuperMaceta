  ...
  55  0003 00            	dc.b	0
  56  0004 00            	dc.b	0
 878                     ; 263 void DHT11_Init( DHT11_t_ptr dht11, DHT11_fPtr Lectura, Timeout_t_ptr Timeout )
 878                     ; 264 {
 880                     .text:	section	.text,new
 881  0000               _DHT11_Init:
 883  0000 89            	pushw	x
 884       00000000      OFST:	set	0
 887                     ; 265 	dht11->Lectura = Lectura;
 889  0001 1605          	ldw	y,(OFST+5,sp)
 890  0003 ef0c          	ldw	(12,x),y
 891                     ; 267 	dht11->Timeout = Timeout;
 893  0005 1607          	ldw	y,(OFST+7,sp)
 894  0007 ef0a          	ldw	(10,x),y
 895                     ; 269 	dht11->Datos.UltimaLectura.CRC = 0;
 897  0009 6f04          	clr	(4,x)
 898                     ; 270 	dht11->Datos.UltimaLectura.T_Decimal = 0;
 900  000b 6f05          	clr	(5,x)
 901                     ; 271 	dht11->Datos.UltimaLectura.T_Entero = 0;
 903  000d 6f06          	clr	(6,x)
 904                     ; 272 	dht11->Datos.UltimaLectura.H_Decimal = 0;
 906  000f 6f07          	clr	(7,x)
 907                     ; 273 	dht11->Datos.UltimaLectura.H_Entero = 0;
 909  0011 6f08          	clr	(8,x)
 910                     ; 275 	NHALgpioConfig_SetType( &dht11->Config.HW, GPIO_MODE_OUT_PP_LOW_SLOW ); //Salida a 2Mhz
 912  0013 4bc0          	push	#192
 913  0015 cd0000        	call	_NHALgpioConfig_SetType
 915  0018 84            	pop	a
 916                     ; 276 	NHALgpio_Init( &dht11->Config.HW );
 918  0019 1e01          	ldw	x,(OFST+1,sp)
 919  001b cd0000        	call	_NHALgpio_Init
 921                     ; 278 	NHALgpio_Write( &dht11->Config.HW, true );
 923  001e 4b01          	push	#1
 924  0020 1e02          	ldw	x,(OFST+2,sp)
 925  0022 cd0000        	call	_NHALgpio_Write
 927  0025 84            	pop	a
 928                     ; 280 	dht11->Datos.Estado = dht11_INICIALIZADO;
...
1313                     ; 133 	NHALgpioConfig_SetType( &sensor->Config.HW, GPIO_MODE_IN_PU_NO_IT); //Configuro como entrada para leer
1315  0043 4b40          	push	#64
1316  0045 1e0d          	ldw	x,(OFST-6,sp)
1317  0047 cd0000        	call	_NHALgpioConfig_SetType
1319  004a 84            	pop	a
1320                     ; 134 	NHALgpio_Init( &sensor->Config.HW ); 
...
1335  0057 9d            	nop	
1336                     
1338                     ; 137 	Timeout_Start( sensor->Timeout, TIMEOUT_1MS );
...
1349  0066               L3201:
1350                     ; 140 		sensor->Datos.Estado = dht11_ESPERA_BAJO;
1352  0066 1e0c          	ldw	x,(OFST-7,sp)
1353  0068 a607          	ld	a,#7
1354  006a e709          	ld	(9,x),a
1355  006c               L5201:
1356                     ; 138 	while( !NHALgpio_Read( &sensor->Config.HW ) && ( sensor->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) )
...
1371  0080               L1301:
1372                     ; 143 	if( sensor->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
1410  00af               L7301:
1411                     ; 155 			sensor->Datos.Estado = dht11_ESPERA_ALTO;
1413  00af 1e0c          	ldw	x,(OFST-7,sp)
1414  00b1 a608          	ld	a,#8
1415  00b3 e709          	ld	(9,x),a
1416  00b5               L1401:
1417                     ; 153 		while( NHALgpio_Read( &sensor->Config.HW ) && ( sensor->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) )
...
1432  00c9               L5401:
1433                     ; 157 		if( sensor->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
1504                     ; 186 	sensor->Datos.Estado = dht11_MIDIENDO_H;
1506  0110 1e0c          	ldw	x,(OFST-7,sp)
1507  0112 a603          	ld	a,#3
1508  0114 e709          	ld	(9,x),a
1509                     ; 76 	uint8_t contador = 0;
1512  0116 0f02          	clr	(OFST-17,sp)
1514                     ; 77 	uint8_t temp = 0;
1516  0118 0f01          	clr	(OFST-18,sp)
1518                     ; 78 	uint8_t bitLeido = 0;
1520                     ; 80 	for( contador; contador < 8; contador++ )
1523  011a 206b          	jra	L3601
1524  011c               L7501:
1525                     ; 82 		Timeout_Start( dht11->Timeout, TIMEOUT_1MS );
...
1533  0127 85            	popw	x
1535  0128               L1701:
1536                     ; 83 		while( !NHALgpio_Read( &dht11->Config.HW ) && ( dht11->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) ) //Espera nuevo bit
...
1551  013c               L5701:
1552                     ; 87 		if( dht11->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
1590                     
1592                     ; 97 			bitLeido = 0;
1594                     ; 98 			if( NHALgpio_Read( &dht11->Config.HW ) )
...
1679  01a8 85            	popw	x
1681  01a9               L1311:
1682                     ; 83 		while( !NHALgpio_Read( &dht11->Config.HW ) && ( dht11->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) ) //Espera nuevo bit
...
1697  01bd               L5311:
1698                     ; 87 		if( dht11->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
1736                     
1738                     ; 97 			bitLeido = 0;
1740                     ; 98 			if( NHALgpio_Read( &dht11->Config.HW ) )
...
1825  0229 85            	popw	x
1827  022a               L1711:
1828                     ; 83 		while( !NHALgpio_Read( &dht11->Config.HW ) && ( dht11->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) ) //Espera nuevo bit
...
1843  023e               L5711:
1844                     ; 87 		if( dht11->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
1882                     
1884                     ; 97 			bitLeido = 0;
1886                     ; 98 			if( NHALgpio_Read( &dht11->Config.HW ) )
...
1971  02aa 85            	popw	x
1973  02ab               L1321:
1974                     ; 83 		while( !NHALgpio_Read( &dht11->Config.HW ) && ( dht11->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) ) //Espera nuevo bit
...
1989  02bf               L5321:
1990                     ; 87 		if( dht11->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
2028                     
2030                     ; 97 			bitLeido = 0;
2032                     ; 98 			if( NHALgpio_Read( &dht11->Config.HW ) )
...
2112  0325 85            	popw	x
2114  0326               L1721:
2115                     ; 83 		while( !NHALgpio_Read( &dht11->Config.HW ) && ( dht11->Timeout->Config.Notificacion( ) < TIMEOUT_MAX_500MS ) ) //Espera nuevo bit
...
2130  033a               L5721:
2131                     ; 87 		if( dht11->Timeout->Config.Notificacion( ) >= TIMEOUT_MAX_500MS )
...
2169                     
2171                     ; 97 			bitLeido = 0;
2173                     ; 98 			if( NHALgpio_Read( &dht11->Config.HW ) )
...
2258                     ; 203 	if( checkCRC == semilla.CRC )
2260  03af 5f            	clrw	x
...
2267                     ; 205 		sensor->Datos.Estado = dht11_LECTURA_OK;
2269  03bb 1e0c          	ldw	x,(OFST-7,sp)
2270  03bd a60b          	ld	a,#11
2271                     ; 206 		return semilla;
2274  03bf 2004          	jra	L316
2275  03c1               L7131:
2276                     ; 210 		sensor->Datos.Estado = dht11_FALLO_AL_LEER;
2278  03c1 1e0c          	ldw	x,(OFST-7,sp)
2279  03c3 a609          	ld	a,#9
2280                     ; 211 		return semilla;
2284  03c5               L316:
